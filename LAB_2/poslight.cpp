...
void init(GLWrapper *glw)
{
	...
	aprism.makeprism(vec3(0.1,0.4,0.5));
...
void display()
{
	...
	model.push(model.top());
	{
		...
		else if(open_lid == false && light_y >= 0.2) light_y -= light_rise_speed *3;

		model.top() = translate(model.top(), vec3(light_x, light_height, light_z));
	...
	draw_cube(model, normalmatrix, view, 0.f, 0.f, 0.f, 1.f, 0.05f, 1.f);
	...
	// This block of code draws the prism which is the angel inside the box
	model.push(model.top());
	{
		...
		// here we scale the model to make it into a cuboid
		model.top() = scale(model.top(), vec3(0.125f, 0.25f, 0.125f));
